...
use crate::{error::AmmError, withdraw};
...
impl<'info> Withdraw<'info> {
    pub fn withdraw(
        &mut self, 
        amount: u64, //amt pf lp tokens that the user wants to burn
        min_x: u64, //min amt of token X that the user wants to receive
        min_y: u64  //min amt of token X that the user wants to receive
    ) -> Result<()> {
        require!(self.config.locked == false, AmmError::PoolLocked);
        require!(amount != 0, AmmError::InvalidAmount);
        // to validate that atleast minimun amt of tokens should be there 
        ...
        // if we don't burn the user can withdraw forever
